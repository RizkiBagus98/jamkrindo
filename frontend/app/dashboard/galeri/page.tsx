                ...
                <tr key={item._id} className="border-b border-gray-200 hover:bg-gray-50">
                  ...
                  <td className="p-3 align-top">
                    <div className="flex gap-2 justify-center">
                      <button onClick={() => openModalForEdit(item)} className="p-2 rounded-full hover:bg-yellow-100 text-yellow-600" title="Edit">
                        <PencilIcon className="w-5 h-5" />
                      </button>
                      <button onClick={() => handleDelete(item._id)} className="p-2 rounded-full hover:bg-red-100 text-red-600" title="Hapus">
          ...
          <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-lg">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-6">
                ...
                <input type="file" onChange={(e) => setImage(e.target.files ? e.target.files[0] : null)} className="w-full p-2 border rounded-md" required={!editingId} />
                {editingId && <p className="text-sm text-gray-500 mt-1">Kosongkan jika tidak ingin mengubah gambar.</p>}
              </div>
              <div className="flex justify-end gap-4">
                <button type="button" onClick={closeModal} className="bg-gray-500 text-white px-4 py-2 rounded-md hover:bg-gray-600">Batal</button>
                <button type="submit" className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700">{editingId ? "Perbarui" : "Simpan"}</button>
              </div>
            </form>
          </div>
        </div>
