...
const BeritaManagementPage = () => {
  ...
  return (
    <>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr key={berita._id} className="border-b border-gray-200 hover:bg-gray-50">
                    <td className="p-3">
                      <img src={`http://localhost:5000${berita.image}`} alt={berita.title} className="w-24 h-16 object-cover rounded-md" />
                    </td>
                    ...
                    <td className="p-3">
                      <div className="flex gap-2 justify-center">
                        <button onClick={() => openEditModal(berita)} className="p-2 rounded-full hover:bg-yellow-100 text-yellow-600" title="Edit">
                          <PencilIcon className="w-5 h-5" />
                        </button>
                        <button onClick={() => handleDelete(berita._id)} className="p-2 rounded-full hover:bg-red-100 text-red-600" title="Hapus">
      ...
      {/* Modal Form */}
      <Modal isOpen={isModalOpen} onClose={closeModal} title={modalMode === 'add' ? 'Tambah Berita Baru' : 'Edit Berita'}>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-3 pt-4">
            <button type="button" onClick={closeModal} className="bg-gray-200 text-gray-800 px-5 py-2.5 rounded-lg font-semibold hover:bg-gray-300">
              Batal
            </button>
            <button type="submit" disabled={submitLoading} className={`inline-flex items-center justify-center bg-[#0725ac] text-white px-5 py-2.5 rounded-lg font-semibold transition ${
